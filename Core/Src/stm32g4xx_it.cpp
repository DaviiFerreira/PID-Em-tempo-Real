...
volatile uint8_t buttonState = 0;     // estado atual
volatile uint8_t buttonLast = 0;      // estado anterior
volatile uint8_t buttonEvent = 0;     // flag para indicar clique detectado
	...
	//pra testar com renode
	 uint8_t raw = HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_13);
	    if (raw != buttonLast) {
	        ...
	        raw = HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_13);
	        if (raw != buttonLast) {
	            buttonLast = raw;

	            // Detecta borda de descida (pressionou)
	            if (raw == GPIO_PIN_RESET) {
	                buttonEvent = 1;  // Marca que o botão foi pressionado
